"""RAG chat API with document upload, embedding, and retrieval."""
import os
import json
import logging
import uuid
...
def get_llm_client(provider: str = None, model: str = None):
    """Return a LlamaIndex LLM client for the given provider/model."""
    provider = (provider or Config.LLM_PROVIDER).lower()
    model = model or Config.LLM_MODEL

    # Fall back to provider-specific default model
...
# Per-user model preferences, falls back to .env defaults
_user_model_prefs: dict[int, tuple[str, str]] = {}


def get_runtime_provider_model(user_id: int = None):
    """Get the active provider and model for a user."""
    if user_id and user_id in _user_model_prefs:
        return _user_model_prefs[user_id]
...
app = FastAPI(
    title="RAG Chat API",
    description="Multi-model RAG system with hybrid search and streaming chat.",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
...
async def global_exception_handler(request, exc):
    """Return JSON errors instead of HTML tracebacks."""
                ...
                with get_db() as conn:
                    with conn.cursor() as cur:
                        ...
                        if retry_count + 1 >= max_retries:
                            ...
                            logger.warning("Job %d dead-lettered after %d retries", job_id, retry_count + 1)
                        else:
                            # Re-queue for retry
                            cur.execute("""
                                UPDATE jobs
...
def ensure_chunks_table():
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            """)
            # Vector index (savepoint in case table is empty)
            ...
            cur.execute("""
                ...
                END $$;
            """)
            # Migrate 'english' tsvector to 'simple' if needed
            cur.execute("""
                DO $$
                DECLARE
                    col_default text;
...
def forgot_password(request: ForgotPasswordRequest):
    ...
    if token:
        ...
        if not email_sent:
            # SMTP not configured or failed — log to console for dev
...
async def upload_document(project_id: int, files: List[UploadFile] = File(...), current_user: dict = Depends(get_current_user)):
    """Upload files and enqueue them for background processing."""
    # Verify user owns the project
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT user_id FROM projects WHERE id = %s", (project_id,))
...
def delete_document(doc_id: int, current_user: dict = Depends(get_current_user)):
    """Delete a document and clean up its file."""
...
def reprocess_document(doc_id: int, current_user: dict = Depends(get_current_user)):
    """Re-process a failed document from scratch."""
    with get_db() as conn:
        with conn.cursor() as cur:
...
async def generate_conversation_title(message: str, user_id: int = None) -> str:
    """Use LLM to generate a 3-word title for the conversation"""
    provider, model = get_runtime_provider_model(user_id)
...
def get_chunk_details(chunk_id: int, current_user: dict = Depends(get_current_user)):
    """Get full content and metadata for a chunk."""
    with get_db() as conn:
        with conn.cursor() as cur:
...
async def summarize_conversation(messages: List[dict], user_id: int = None) -> str:
...
async def chat(request: ChatRequest = None, current_user: dict = Depends(get_current_user)):
    """Stream a RAG response with citations via SSE."""
    ...
    if len(history) > Config.SUMMARIZE_AFTER_MESSAGES:
        ...
        summary = await summarize_conversation(older_messages, current_user["id"])
        if summary:
    ...
    if is_first_message and current_title == "New Conversation":
        try:
            new_title = await generate_conversation_title(request.message, current_user["id"])
        except Exception:
    ...
    # Skip retrieval for follow-up queries
    follow_up_patterns = [
    ...
    async def generate():
        ...
        provider, model = get_runtime_provider_model(current_user["id"])
        llm = get_llm_client(provider, model)

        # Tell the frontend which model is answering
        yield f"event: model_info\ndata: {json.dumps({'provider': provider, 'model': model})}\n\n"
...
def export_conversation(
    ...
    """Export a conversation as Markdown, PDF, or JSON."""
...
def get_models(current_user: dict = Depends(get_current_user)):
    ...
    # Include unconfigured providers as unavailable
...
def select_model(request: ModelSelectRequest, current_user: dict = Depends(get_current_user)):
    """Switch the active LLM provider and model for the current user."""
    ...
    _user_model_prefs[current_user["id"]] = (provider, model)
    logger.info("User %d switched model to %s/%s", current_user["id"], provider, model)
